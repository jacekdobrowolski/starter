...
void USART_IrqHandler (UART_HandleTypeDef *huart, DMA_HandleTypeDef *hdma)
{
	if (huart->Instance->SR & UART_FLAG_IDLE)           /* if Idle flag is set */
	{
        ...
        tmp = huart->Instance->HISR;                       /* Read status register */
        tmp = huart->Instance->HIDR;                       /* Read data register */
		hdma->Instance->HICR &= ~DMA_SxCR_EN;       /* Disabling DMA will force transfer complete interrupt if enabled */       
	}
}
